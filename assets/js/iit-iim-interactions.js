...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 4b. SDG Filter Buttons (matching index.html classes)
    const sdgFilterBtns = document.querySelectorAll(".sdg-filter-btn");
    const sdgItems = document.querySelectorAll(".sdg-item");

    sdgFilterBtns.forEach(btn => {
        btn.addEventListener("click", function () {
            sdgFilterBtns.forEach(b => b.classList.remove("active"));
            btn.classList.add("active");
            const filter = btn.getAttribute("data-filter");

            sdgItems.forEach((item, i) => {
